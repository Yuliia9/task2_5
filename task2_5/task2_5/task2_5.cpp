...
int main()
{
	...
	printf("List will consist of %u points.\n", num);
...
void Interface()
{
	...
	printf("Program will sort your list \nand define for you smallest distance between two points of same color.\n");
...
unsigned char Input(struct point* points, unsigned int num)
{
	...
	for (i = 0; i < num; ++i)
	{
		...
		while (token != NULL && coord_count < FIELDS)
		{
			...
			{
				printf("Type mismatch. Next time please check if you entered float or integer numbers \n");
				fflush(stdin);
				break;
			}
